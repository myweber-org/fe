...
mod tests {
    ...
    fn create_test_csv() -> NamedTempFile {
        ...
        writeln!(file, "1,Alpha,42.5,TypeA").unwrap();
        writeln!(file, "2,Beta,37.8,TypeB").unwrap();
        writeln!(file, "3,Gamma,55.2,TypeA").unwrap();
        writeln!(file, "4,Delta,29.1,TypeC").unwrap();
        file
    }
    ...
    fn test_load_and_filter() {
        ...
        assert!((avg - 41.15).abs() < 0.01);
        
        let max_record = processor.find_max_value().unwrap();
        assert_eq!(max_record.name, "Gamma");
    }
}