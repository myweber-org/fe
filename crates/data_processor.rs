...
impl DataProcessor {
    ...
    pub fn export_json(&self) -> String {
        let items: Vec<String> = self.records.iter().map(|record| record.to_json()).collect();
        format!("[{}]", items.join(","))
    }

    pub fn statistics(&self) -> (usize, Option<f64>, usize) {
        let total = self.records.len();
        let avg = self.calculate_average();
        let unique_categories = self.count_unique_categories();
        (total, avg, unique_categories)
    }

    fn count_unique_categories(&self) -> usize {
...
mod tests {
    ...
    fn test_data_processor() {
        ...
        assert!((stats.mean - 21.54).abs() < 0.01);
...
impl DataProcessor {
    ...
    pub fn get_statistics(&self) -> Statistics {
        ...
        let average_value = self.calculate_average().unwrap_or(0.0);
...
pub fn filter_records(records: Vec<DataRecord>, min_id: u32) -> Vec<DataRecord> {
    records.into_iter()
...
mod tests {
    ...
    fn test_filter_records() {
        let records = vec![
            DataRecord::new(1, 1000, vec![1.0]),
            DataRecord::new(5, 2000, vec![2.0]),
            DataRecord::new(10, 3000, vec![3.0]),
        ];
        
        let filtered = filter_records(records, 5);
        assert_eq!(filtered.len(), 2);
        assert!(filtered.iter().all(|r| r.id >= 5));
    }
}