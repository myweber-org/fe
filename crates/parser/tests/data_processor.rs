...
mod tests {
    ...
    fn test_data_processing() {
        ...
        assert!((stats["std_dev"] - 4.93).abs() < 0.1);
...
pub fn process_records(records: &mut [DataRecord], multiplier: f64) -> Vec<Result<f64, ValidationError>> {
    records
        .iter_mut()
        .map(|record| {
            record.validate()?;
...
mod tests {
    ...
    fn test_valid_record() {
        ...
        assert_eq!(record.calculate_score(), 42.5 * 100.0 + 20.0);
    }
...
pub fn process_csv_file(file_path: &Path) -> Result<Vec<DataRecord>, Box<dyn Error>> {
    ...
    for line in reader.lines() {
        ...
        let record = DataRecord::new(id, value, parts[2])
            .map_err(|e| format!("Error at line {}: {}", line_number, e))?;
...
mod tests {
    ...
    fn test_process_csv_file() {
        ...
        writeln!(temp_file, "2,33.7,category_b").unwrap();
        writeln!(temp_file, "# This is a comment").unwrap();
        writeln!(temp_file, "").unwrap();
        writeln!(temp_file, "3,15.2,category_c").unwrap();
...
mod tests {
    ...
    fn test_data_record_validation() {
        let valid_record = DataRecord::new(1, 42.5, "2024-01-01".to_string(), "A".to_string());
        assert!(valid_record.is_valid());
    ...
    fn test_load_from_csv() {
        let mut csv_content = "id,value,timestamp,category\n".to_string();
        csv_content.push_str("1,42.5,2024-01-01,A\n");
        csv_content.push_str("2,37.8,2024-01-02,B\n");
        csv_content.push_str("invalid,data,row\n");
...
mod tests {
    ...
    fn test_data_processor() {
        ...
        assert!((average - 23.75).abs() < 0.001);
...
mod tests {
    ...
    fn test_extract_column() {
        ...
        assert!(processor.extract_column(&records, 5).is_err());
    }
}use csv::Reader;
use serde::Deserialize;
use std::error::Error;
...
impl DataProcessor {
    ...
    pub fn calculate_statistics(&self, records: &[Vec<String>], column_index: usize) -> Result<Statistics, Box<dyn Error>> {
        let mut values = Vec::new();
