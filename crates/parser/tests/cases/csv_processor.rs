            ...
            if line_content.trim().is_empty() {
                if self.strict_mode {
                    return Err(CsvError::ParseError(
                        "Empty line in strict mode".to_string(),
                        line_number,
                    ));
                }
                continue;
...
mod tests {
    ...
    fn test_valid_csv_processing() {
        ...
        writeln!(temp_file, "3,Bob Johnson,29.3,true").unwrap();
        ...
        let stats = CsvProcessor::calculate_statistics(&records);
        assert!((stats.0 - 36.533333).abs() < 0.001);
        assert_eq!(stats.2, 2);
    }
    ...
    fn test_invalid_csv_format() {
        let mut temp_file = NamedTempFile::new().unwrap();
        writeln!(temp_file, "1,John Doe,42.5").unwrap();

        let processor = CsvProcessor::default();
        let result = processor.process_file(temp_file.path());
        
        assert!(result.is_err());
        if let Err(CsvError::ParseError(_, line)) = result {
            assert_eq!(line, 1);
        } else {
            panic!("Expected ParseError");
        }
