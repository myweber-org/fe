...
impl PasswordGenerator {
    ...
    pub fn generate(&self) -> Result<String, &'static str> {
        ...
        for _ in 0..self.length {
            ...
            if ch.is_lowercase() {
                used_sets.insert("lowercase");
            } else if ch.is_uppercase() {
                used_sets.insert("uppercase");
            } else if ch.is_digit() {
                used_sets.insert("digits");
            } else {
                used_sets.insert("special");
            }
        }
        ...
        for (enabled, set_name) in &required_sets {
            if *enabled && !used_sets.contains(*set_name) {
...
mod tests {
    ...
    #[test]
    fn test_character_set_exclusion() {
        let generator = PasswordGenerator::new(15)
            .digits(false)
            .special(false);
