    ...
    pub fn filter_by_column(&self, column_name: &str, predicate: fn(&str) -> bool) -> Vec<Vec<String>> {
        ...
        if let Some(col_index) = self.headers.iter().position(|h| h == column_name) {
            for record in &self.records {
                if record.get(col_index).map_or(false, |value| predicate(value)) {
...
mod tests {
    ...
    fn test_csv_processing() {
        ...
        let adults = processor.filter_by_column("age", |age| {
            age.parse::<u32>().map_or(false, |a| a >= 30)
        });
        
        assert_eq!(adults.len(), 2);
        assert!(adults.iter().any(|r| r[0] == "Alice"));
        assert!(adults.iter().any(|r| r[0] == "Charlie"));
    }
}