...
pub fn validate_csv_data(stats: &CsvStats) -> Vec<String> {
    ...
    for (i, row) in stats.sample_data.iter().enumerate() {
        if row.len() != stats.column_count {
            issues.push(format!("Row {} has {} columns, expected {}", i + 1, row.len(), stats.column_count));
        }
    }
    ...
    fn test_validate_csv() {
        let stats = CsvStats {
            ...
            sample_data: vec![
                ...
                vec!["x".to_string(), "y".to_string()], // Invalid row
            ],
        };
        
        let issues = validate_csv_data(&stats);
        assert!(issues.contains(&"Row 2 has 2 columns, expected 3".to_string()));
    }
}