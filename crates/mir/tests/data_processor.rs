    ...
    pub fn validate_numeric_fields(&self, records: &[Vec<String>], field_index: usize) -> Result<Vec<f64>, Box<dyn Error>> {
        ...
        for (index, record) in records.iter().enumerate() {
            if field_index >= record.len() {
                return Err(format!("Field index out of bounds at line {}", index + 1).into());
            }

            match record[field_index].parse::<f64>() {
                Ok(value) => numeric_values.push(value),
                Err(_) => return Err(format!("Non-numeric value at line {} field {}", index + 1, field_index).into()),
            }
        }
...
mod tests {
    ...
    fn test_numeric_validation() {
        ...
        let numeric_data = processor.validate_numeric_fields(&records, 1).unwrap();

        assert_eq!(numeric_data, vec![42.5, 18.0]);
    }
}