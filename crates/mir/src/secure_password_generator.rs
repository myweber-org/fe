...
impl PasswordGenerator {
    ...
    pub fn generate(&self) -> Result<String, &'static str> {
        ...
        if self.use_lowercase && !used_chars.iter().any(|c| c.is_lowercase()) {
            missing_sets.push("lowercase");
        }
        
        if self.use_uppercase && !used_chars.iter().any(|c| c.is_uppercase()) {
            missing_sets.push("uppercase");
        }
        
        if self.use_digits && !used_chars.iter().any(|c| c.is_digit(10)) {
            missing_sets.push("digits");
        }
        
        if self.use_special && !used_chars.iter().any(|c| !c.is_alphanumeric()) {
            missing_sets.push("special");
        }

        if !missing_sets.is_empty() {
            return Err("Failed to include all enabled character sets");
        }
