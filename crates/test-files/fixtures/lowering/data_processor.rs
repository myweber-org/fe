...
mod tests {
    ...
    fn test_data_processor() {
        ...
        let min_record = processor.get_min_value();
        assert!(min_record.is_some());
        assert_eq!(min_record.unwrap().id, 1);
...
pub fn process_records(records: &mut Vec<DataRecord>) -> HashMap<String, f64> {
    ...
    for record in records.iter_mut() {
        if record.validate().is_ok() {
            ...
            for tag in &record.tags {
                *summary.entry(tag.clone()).or_insert(0.0) += record.value;
            }
        }
    }

    if valid_count > 0 {
        summary.insert("average".to_string(), total_value / valid_count as f64);
    }

    summary.insert("total".to_string(), total_value);
    summary.insert("valid_count".to_string(), valid_count as f64);
...
mod tests {
    ...
    #[test]
    fn test_record_validation() {
        let valid_record = DataRecord::new(1, "test".to_string(), 100.0, vec!["tag1".to_string()]);
        assert!(valid_record.validate().is_ok());

        let invalid_record = DataRecord::new(2, "".to_string(), -10.0, vec![]);
        assert!(invalid_record.validate().is_err());
    }

    #[test]
    fn test_record_transformation() {
        let mut record = DataRecord::new(1, "test".to_string(), 100.0, vec![]);
    ...
    fn test_process_records() {
        let mut records = vec![
            DataRecord::new(1, "record1".to_string(), 100.0, vec!["tag1".to_string()]),
            DataRecord::new(2, "record2".to_string(), 200.0, vec!["tag1".to_string(), "tag2".to_string()]),
        ];
    ...
    pub fn validate_numeric_fields(&self, records: &[Vec<String>], field_index: usize) -> Result<Vec<f64>, Box<dyn Error>> {
        ...
        for (row_index, record) in records.iter().enumerate() {
            if field_index >= record.len() {
                return Err(format!("Field index out of bounds at row {}", row_index + 1).into());
            }
            
            match record[field_index].parse::<f64>() {
                Ok(value) => numeric_values.push(value),
                Err(_) => return Err(format!("Non-numeric value at row {} field {}", row_index + 1, field_index).into()),
            }
        }
...
mod tests {
    ...
    fn test_csv_loading() {
        ...
        writeln!(temp_file, "5,12.0,").unwrap();
...
impl DataProcessor {
    ...
    pub fn calculate_statistics(&self, records: &[Vec<String>], column_index: usize) -> Result<(f64, f64), Box<dyn Error>> {
        if records.is_empty() {
            return Err("No records available for statistics".into());
        }
