...
pub fn process_directory(dir_path: &Path, password: &str, encrypt: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for entry in fs::read_dir(dir_path)? {
        ...
        if path.is_file() {
            ...
            if !extension.is_empty() && extension != "enc" {
                let output_path = if encrypt {
                    path.with_extension(format!("{}.enc", extension))
                } else if extension.ends_with(".enc") {
                    let base = &extension[..extension.len() - 4];
                    path.with_extension(base)
                } else {
                    continue;
                };
