...
mod tests {
    ...
    fn test_category_statistics() {
        ...
        assert_eq!(stats.get("A").unwrap().count, 2);
        assert_eq!(stats.get("A").unwrap().average_value, 150.0);
        assert_eq!(stats.get("B").unwrap().count, 1);
    }
}use std::error::Error;
use std::fs::File;
use std::io::{BufRead, BufReader};
...
mod tests {
    ...
    fn test_invalid_record() {
        let record1 = DataRecord::new(0, 42.5, "2024-01-15T10:30:00Z".to_string());
        let record2 = DataRecord::new(1, f64::INFINITY, "2024-01-15T10:30:00Z".to_string());
    ...
    fn test_process_csv() -> Result<(), Box<dyn Error>> {
        ...
        writeln!(temp_file, "2,38.2,2024-01-15T11:00:00Z")?;
    ...
    fn test_calculate_statistics() {
        ...
        assert!((variance - 66.666666).abs() < 0.0001);
        assert!((std_dev - 8.164966).abs() < 0.0001);
    }
}