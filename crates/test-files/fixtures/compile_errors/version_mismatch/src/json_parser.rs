...
impl JsonParser {
    ...
    fn parse_bool(&mut self) -> Result<JsonValue, String> {
        if self.input.get(self.pos..self.pos + 4) == Some(&['t', 'r', 'u', 'e']) {
            self.pos += 4;
            Ok(JsonValue::Bool(true))
        } else if self.input.get(self.pos..self.pos + 5) == Some(&['f', 'a', 'l', 's', 'e']) {
            self.pos += 5;
            Ok(JsonValue::Bool(false))
        } else {
...
impl JsonParser {
    ...
    fn parse_number(&mut self) -> Result<JsonValue, String> {
        ...
        if self.pos < self.input.len() && self.input.chars().nth(self.pos) == Some('0') {
            self.pos += 1;
        } else {
            while self.pos < self.input.len() {
                let c = self.input.chars().nth(self.pos).unwrap();
        ...
        if self.pos < self.input.len() && (self.input.chars().nth(self.pos) == Some('e') || 
                                           self.input.chars().nth(self.pos) == Some('E')) {
            has_exponent = true;
            self.pos += 1;
            if self.pos < self.input.len() && (self.input.chars().nth(self.pos) == Some('+') || 
