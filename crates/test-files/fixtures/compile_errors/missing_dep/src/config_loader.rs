...
mod tests {
    ...
    fn test_config_loading() {
        ...
        writeln!(file, "FEATURE_CACHE=true").unwrap();
        writeln!(file, "# This is a comment").unwrap();
        writeln!(file, "FEATURE_ANALYTICS=false").unwrap();
    ...
    fn test_env_var_resolution() {
        ...
        let mut file = NamedTempFile::new().unwrap();
        writeln!(file, "DATABASE_URL=postgres://${DB_HOST}:5432/app").unwrap();
        
        let config = Config::from_file(file.path().to_str().unwrap()).unwrap();
        assert_eq!(config.database_url, "postgres://database.internal:5432/app");
    }
}