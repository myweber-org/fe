...
mod tests {
    ...
    fn test_filter_by_category() {
        ...
        processor.records.push(DataRecord::new(3, 15.0, "alpha".to_string()).unwrap());
...
mod tests {
    ...
    fn test_average_calculation() {
        ...
        let average = DataProcessor::calculate_average(&numbers).unwrap();
        assert_eq!(average, 3.0);
        
        let empty: Vec<f64> = vec![];
        assert!(DataProcessor::calculate_average(&empty).is_none());
    }
}
use std::error::Error;
use std::fs::File;
