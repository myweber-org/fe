...
mod tests {
    ...
    fn test_encryption_decryption() {
        ...
        let encrypted_path = output_encrypted.path().to_str().unwrap();

        let result = encrypt_file(input_path, encrypted_path, password).unwrap();

        let output_decrypted = NamedTempFile::new().unwrap();
        let decrypted_path = output_decrypted.path().to_str().unwrap();
