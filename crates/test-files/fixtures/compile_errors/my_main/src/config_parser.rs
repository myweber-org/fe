...
mod tests {
    ...
    fn test_env_interpolation() {
        ...
        let mut file = NamedTempFile::new().unwrap();
        writeln!(file, "DB_URL=postgres://${DB_USER}:${DB_PASS}@localhost").unwrap();

        let config = Config::from_file(file.path().to_str().unwrap()).unwrap();
        assert_eq!(
            config.get("DB_URL"),
            Some(&"postgres://admin:secret123@localhost".to_string())
        );
    }
}
use std::collections::HashMap;
...
mod tests {
    ...
    fn test_env_substitution() {
        ...
        let mut file = NamedTempFile::new().unwrap();
        writeln!(file, "SECRET=${{APP_SECRET}}").unwrap();
        writeln!(file, "PATH=/api/${{APP_SECRET}}/endpoint").unwrap();

        let config = Config::from_file(file.path().to_str().unwrap()).unwrap();
        assert_eq!(config.get("SECRET"), Some(&"super-secret-value".to_string()));
        assert_eq!(config.get("PATH"), Some(&"/api/super-secret-value/endpoint".to_string()));
    }
}