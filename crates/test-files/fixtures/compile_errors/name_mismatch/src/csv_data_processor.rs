...
pub fn transform_records<F>(records: &[CsvRecord], transform_fn: F) -> Vec<CsvRecord>
where
    F: Fn(&CsvRecord) -> CsvRecord,
{
    records.iter().map(transform_fn).collect()
}
...
mod tests {
    ...
    fn test_transform_records() {
        ...
        let transformed = transform_records(&records, |r| {
            CsvRecord::new(r.id, r.name.clone(), r.value * 2.0, r.category.clone())
        });
