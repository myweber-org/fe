...
pub fn decrypt_file(
    ...
    if nonce.len() != NONCE_SIZE {
        return Err(format!("Invalid nonce size: expected {}", NONCE_SIZE));
    }
...
pub fn decrypt_file(
    ...
    let decrypted_data = decrypt_data(ciphertext, password, &nonce, &salt)?;
...
mod tests {
    ...
    fn test_encryption_decryption() {
        ...
        let result = encrypt_data(data, password).unwrap();
...
pub fn derive_key(password: &str, salt: &SaltString) -> io::Result<Key<Aes256Gcm>> {
    ...
    let key_slice = &hash_bytes[..32];
    let key_array: [u8; 32] = key_slice
        .try_into()
        .map_err(|_| io::Error::new(io::ErrorKind::Other, "Key conversion failed"))?;
...
pub fn encrypt_data(data: &[u8], password: &str) -> io::Result<EncryptionResult> {
    ...
    let ciphertext = cipher
        .encrypt(nonce_obj, combined_data.as_ref())
        .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
...
pub fn decrypt_data(result: &EncryptionResult, password: &str) -> io::Result<Vec<u8>> {
    let salt_bytes = &result.ciphertext[..SaltString::RECOMMENDED_LENGTH];
    ...
    let plaintext = cipher
        .decrypt(nonce_obj, &result.ciphertext[SaltString::RECOMMENDED_LENGTH..])
        .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
...
pub fn decrypt_file(input_path: &Path, output_path: &Path, password: &str) -> io::Result<()> {
    ...
    let nonce = buffer[..NONCE_SIZE].try_into().unwrap();
    let ciphertext = buffer[NONCE_SIZE..].to_vec();

    let result = EncryptionResult { ciphertext, nonce };
