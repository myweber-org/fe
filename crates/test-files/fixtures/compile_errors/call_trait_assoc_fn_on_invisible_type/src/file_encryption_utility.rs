...
pub fn derive_key(password: &str, salt: &[u8]) -> Result<Key<Aes256Gcm>, String> {
    ...
    if key_bytes.len() != 32 {
        return Err("Derived key length mismatch".to_string());
    }
...
mod tests {
    ...
    fn test_file_encryption() {
        ...
        encrypt_file(temp_input.path(), temp_output.path(), password)
            .expect("File encryption failed");

        decrypt_file(temp_output.path(), temp_decrypted.path(), password)
