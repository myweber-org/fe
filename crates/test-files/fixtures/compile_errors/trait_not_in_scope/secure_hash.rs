...
mod tests {
    ...
    fn test_hash_consistency() -> Result<()> {
        ...
        let hash1 = calculate_file_hash(temp_file.path())?;
        let hash2 = calculate_file_hash(temp_file.path())?;
    ...
    fn test_verification() -> Result<()> {
        ...
        let calculated_hash = calculate_file_hash(temp_file.path())?;
        let verification_result = verify_file_integrity(temp_file.path(), &calculated_hash)?;
