...
mod tests {
    ...
    fn test_load_from_csv() {
        ...
        csv_content.push_str("1,item1,10.5,1000\n");
        csv_content.push_str("2,item2,20.3,2000\n");
        csv_content.push_str("3,item3,30.7,3000\n");
...
mod tests {
    ...
    fn test_validation_failure() {
        ...
        let invalid_data = vec![920.5, 980.2, 1100.7];
    ...
    pub fn validate_numeric_fields(&self, data: &[Vec<String>], column_index: usize) -> Result<Vec<f64>, String> {
        ...
        for (row_idx, row) in data.iter().enumerate() {
            if column_index >= row.len() {
                return Err(format!("Row {}: Column index {} out of bounds", row_idx, column_index));
            }
            
            match row[column_index].parse::<f64>() {
                Ok(value) => numeric_values.push(value),
                Err(_) => return Err(format!("Row {}: Invalid numeric value '{}'", row_idx, row[column_index])),
            }
        }
...
mod tests {
    ...
    fn test_numeric_validation() {
        ...
        let result = processor.validate_numeric_fields(&data, 1).unwrap();
        
        assert_eq!(result, vec![20.0, 25.5]);
    }
