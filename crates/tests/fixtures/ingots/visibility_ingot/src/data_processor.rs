...
mod tests {
    ...
    fn test_validation() {
        ...
        let processor = DataProcessor::new(',', false);
        let valid_count = processor.validate_records(&records);
        assert_eq!(valid_count.unwrap(), 2);
    }
}
use std::error::Error;
use std::fs::File;
...
mod tests {
    ...
    fn test_data_processor() {
        ...
        writeln!(temp_file, "2,15.3,B,2024-01-02").unwrap();
        writeln!(temp_file, "3,-5.0,A,2024-01-03").unwrap();
...
mod tests {
    ...
    fn test_data_processor() {
        ...
        writeln!(temp_file, "3,12.8,A").unwrap();
        ...
        assert!((mean - 12.866666).abs() < 0.001);
        assert!((variance - 4.803333).abs() < 0.001);
    }
}