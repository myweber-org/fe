...
mod tests {
    ...
    fn test_process_data() {
        ...
        let data = vec![0.1, 0.4, 0.2, 0.8, 0.5];
        let result = processor.process_data(&data);
        assert!(result.is_ok());
        let processed = result.unwrap();
        assert_eq!(processed, vec![0.4, 0.8, 0.5]);
    }
    ...
    fn test_calculate_statistics() {
        ...
        let result = processor.calculate_statistics(&data);
        assert!(result.is_ok());
        let (mean, std_dev) = result.unwrap();
        assert!((mean - 0.3).abs() < 0.0001);
        assert!((std_dev - 0.141421).abs() < 0.0001);
    }
}