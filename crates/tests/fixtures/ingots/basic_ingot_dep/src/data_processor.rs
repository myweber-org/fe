...
mod tests {
    ...
    fn test_basic_statistics() {
        ...
        assert!(dataset.calculate_standard_deviation().unwrap() > 8.16);
...
impl DataProcessor {
    ...
    fn validate_data(&self, data: &[f64]) -> Result<Vec<f64>, String> {
        ...
        for &value in data {
            ...
            if !value.is_finite() {
                return Err("Non-finite value in data set".to_string());
            }
            result.push(value);
        }
...
mod tests {
    ...
    fn test_cache_functionality() {
        let mut processor = DataProcessor::new();
        let test_data = [1.5, 2.5, 3.5];
        
        let first_result = processor.process_numeric_data("test", &test_data).unwrap();
        let second_result = processor.process_numeric_data("test", &test_data).unwrap();
