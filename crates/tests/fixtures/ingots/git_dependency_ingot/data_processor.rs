...
mod tests {
    ...
    fn test_data_processor() {
        ...
        assert_eq!(processor.get_average_value(), Some(11.466666666666667));
        
        let type_a_records = processor.filter_by_category("TypeA");
        assert_eq!(type_a_records.len(), 2);
        
        assert_eq!(processor.get_max_value(), Some(15.2));
        assert_eq!(processor.get_min_value(), Some(8.7));
    }
    ...
    fn test_invalid_data() {
        ...
        let mut temp_file = NamedTempFile::new().unwrap();
        writeln!(temp_file, "id,value,category").unwrap();
        writeln!(temp_file, "1,abc,TypeA").unwrap();
        writeln!(temp_file, "2,10.0,").unwrap();
        
        let count = processor.load_from_csv(temp_file.path()).unwrap();
        assert_eq!(count, 0);
        assert_eq!(processor.count_valid_records(), 0);
