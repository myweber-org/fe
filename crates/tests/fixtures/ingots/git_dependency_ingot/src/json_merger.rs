...
mod tests {
    ...
    fn test_merge_json_files() {
        ...
        let expected: Value = serde_json::from_str(r#"{"name": "Alice", "age": 31, "city": "Berlin"}"#).unwrap();
...
mod tests {
    ...
    fn test_merge_json() {
        ...
        let merged = merge_json_files(&[file1.path(), file2.path()]).unwrap();
        let expected = json!({
            "a": 1,
            "b": {"x": 10, "y": 20},
            "c": 3
