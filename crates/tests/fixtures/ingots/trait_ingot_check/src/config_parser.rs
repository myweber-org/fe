...
mod tests {
    ...
    fn test_env_interpolation() {
        ...
        writeln!(file, "NESTED=prefix_${DB_HOST}_suffix").unwrap();
        writeln!(file, "MISSING_VAR=${UNDEFINED}").unwrap();

        let config = Config::from_file(file.path().to_str().unwrap()).unwrap();
        assert_eq!(config.get("DATABASE_URL"), Some(&"postgresql:5432/mydb".to_string()));
        assert_eq!(config.get("NESTED"), Some(&"prefix_postgresql_suffix".to_string()));
        assert_eq!(config.get("MISSING_VAR"), Some(&"".to_string()));
    }
