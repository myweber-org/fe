...
mod tests {
    ...
    fn test_extract_domain_http() {
        let url = "http://api.example.org:8080/data";
        let domain = UrlParser::extract_domain(url);
        assert_eq!(domain, Some("api.example.org:8080".to_string()));
    }

    #[test]
    fn test_extract_domain_no_protocol() {
        let url = "example.net/page";
        let domain = UrlParser::extract_domain(url);
        assert_eq!(domain, Some("example.net".to_string()));
    }
}use regex::Regex;
use std::collections::HashMap;
