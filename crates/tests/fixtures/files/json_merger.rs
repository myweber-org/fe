...
pub fn merge_json_files<P: AsRef<Path>>(paths: &[P]) -> Result<JsonValue, Box<dyn std::error::Error>> {
    let mut merged_array = Vec::new();
...
pub fn deduplicate_json_array(array: JsonValue, key: &str) -> Result<JsonValue, Box<dyn std::error::Error>> {
    if let JsonValue::Array(arr) = array {
        ...
        for item in arr {
            if let JsonValue::Object(ref map) = item {
...
pub fn write_json_file<P: AsRef<Path>>(path: P, json_value: &JsonValue) -> Result<(), Box<dyn std::error::Error>> {
    let pretty_json = serde_json::to_string_pretty(json_value)?;
