...
mod tests {
    ...
    fn test_different_key_fails() {
        ...
        encrypt_file(input_file.path(), encrypted_file.path(), Some(0x42)).unwrap();
        decrypt_file(encrypted_file.path(), decrypted_file.path(), Some(0x99)).unwrap();
        
        let decrypted_data = fs::read(decrypted_file.path()).unwrap();
        assert_ne!(original_data, decrypted_data.as_slice());
    }
}use aes_gcm::{
...
impl FileEncryptor {
    ...
    pub fn from_key(key: &[u8; 32]) -> io::Result<Self> {
        let key = Key::<Aes256Gcm>::from_slice(key);
        let cipher = Aes256Gcm::new(key);
        Ok(Self { cipher })
    }
