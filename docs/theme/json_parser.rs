...
impl JsonParser {
        ...
        while self.pos < self.input.len() && self.input[self.pos] != '"' {
            if self.input[self.pos] == '\\' {
                ...
                match self.input[self.pos] {
                    ...
                    'r' => result.push('\r'),
                    't' => result.push('\t'),
                    _ => return Err(format!("Invalid escape character: {}", self.input[self.pos])),
                }
            } else {
    ...
    fn consume_str(&mut self, s: &str) -> bool {
        if self.pos + s.len() <= self.input.len() {
            let slice: String = self.input[self.pos..self.pos + s.len()].iter().collect();
            if slice == s {
                self.pos += s.len();
                return true;
            }
    ...
    fn expect_char(&mut self, expected: char) -> Result<(), String> {
        ...
        if self.pos < self.input.len() && self.input[self.pos] == expected {
            ...
            Err(format!("Expected '{}', found '{:?}'", expected, 
