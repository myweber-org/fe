...
mod tests {
    ...
    fn test_data_processing() {
        ...
        assert_eq!(mean, 15.166666666666666);
        assert_eq!(min, 10.5);
        assert_eq!(max, 20.0);
...
mod tests {
    ...
    fn test_filtering() {
        let mut processor = DataProcessor::new();
        processor.records.push(DataRecord::new(1, 10.0, "A".to_string(), "ts1".to_string()));
        processor.records.push(DataRecord::new(2, 20.0, "B".to_string(), "ts2".to_string()));
        processor.records.push(DataRecord::new(3, 30.0, "A".to_string(), "ts3".to_string()));
    ...
    fn test_filter_records() {
        let records = vec![
            DataRecord {
                id: 1,
                timestamp: 1,
                values: HashMap::from([("v".into(), 1.0)]),
                tags: vec!["important".into()],
            },
            DataRecord {
                id: 2,
                timestamp: 2,
                values: HashMap::from([("v".into(), 2.0)]),
...
mod tests {
    ...
    fn test_transform_values() {
        ...
        let category_stats = processor.get_category_statistics("A").unwrap();
        assert_eq!(category_stats.total_value, 60.0);
    }
